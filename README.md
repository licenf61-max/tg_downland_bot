...
| t.me 链接解析 | 发 `t.me/频道/123` 直接下载那条消息里的文件（需配置用户账号，见下） |

## 直接下载 t.me 频道消息链接（可选配置）

Bot API 没有"按链接取消息"的接口，要解析 `https://t.me/xxx/472` 这种链接，
机器人会启用一个 **Telethon 用户账号**（你的个人 Telegram 账号，登录一次永久有效）：

1. 到 [my.telegram.org](https://my.telegram.org) → API development tools，免费申请 `api_id` / `api_hash`
2. 填到 `.env` 的 `API_ID` / `API_HASH`
3. 在项目目录双击 **`login_tg.bat`**，按提示输入手机号 → Telegram App 里收验证码 →（开了两步验证的话再输密码）

之后直接发 `t.me/频道名/123` 或私有频道的 `t.me/c/xxxx/123`，机器人会：
- 下载那条消息里的文件（相册整组一起抓，自动编号）
- 优先用你消息里的文字标签命名，其次用频道消息自带的说明
- 走和机器人相同的代理；连接复用，不重复登录

**注意**：
- 私有频道/群需要先用这个账号加入，才能取到消息
- 频道开了「限制保存内容」时仍可能拿不到文件
- 用用户账号跑自动化有理论上的封号风险，建议小规模使用、行为像正常人（不要每秒批量抓）

## 文件按「文字标签」命名（方便索引）

保存时**用你给的文字当文件名**，而不是 `photo_a1b2c3.jpg` 这种随机名。取用顺序：

| 优先级 | 你怎么发 | 存成什么 |
| --- | --- | --- |
| ① | 文件 + 文字说明（caption） | `张三的合同.pdf` |
| ② | 回复某条文字，再把文件发给机器人 | `2024 年报表.pdf` |
| ③ | 发链接时把标签写在旁边：`合同 https://.../a.pdf` | `合同.pdf` |
| ④ | 三种都没有 | 原始文件名（图片等则退回默认名） |

- **相册**（一次发多张）：说明文字通常只挂在其中一条上，机器人会抽出来**整组共用并自动编号** →
  `全家福 2024_1.jpg`、`全家福 2024_2.jpg`、`全家福 2024_3.jpg`
- 标签里的 `/` `\` `:` `*` 等非法字符会替换成 `_`，不会截断内容（`2024/09 月报` → `2024_09 月报.pdf`）
- 标签本身已带扩展名时不重复追加（写 `报告.pdf` 不会变成 `报告.pdf.pdf`）
- 原始扩展名始终保留；标签上限 80 字（超出截断）
- 索引（`data/index.json`）里额外存了一份 `label` 字段，`/search` 会同时匹配文件名和标签
